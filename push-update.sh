...
updates_dir=/data/avium_updates

if [ ! -f "$1" ]; then
   echo "Usage: $0 ZIP [UNVERIFIED]"
   echo "Push ZIP to $updates_dir and add it to Updater"
   echo
   echo "The name of ZIP is assumed to have AviumUIProject-VERSION-DATE-TYPE-* as format"
...
# Assume AviumUIProject-VERSION-DATE-TYPE-*.zip
...
adb shell "killall org.avium.updater 2>/dev/null"
adb shell "sqlite3 /data/data/org.avium.updater/databases/updates.db" \
